...
class Player(Base):
    ...
    id = Column(BigInteger, primary_key=True, index=True) # ID do Telegram (ou ID fake para bots)
    username = Column(String(50), nullable=True)
    name = Column(String(15)) # Limite de 15 caracteres
    class_name = Column(String(20))
    
    # Progresso
    level = Column(Integer, default=1)
    xp = Column(Integer, default=0)
    # Mudança: Ouro inicial será definido na criação, mas o default do banco pode ser 1000
...
# --- Funções de Inicialização e Seeding (Bots) ---

def seed_bots(db):
    """Cria 200 jogadores falsos para popular o ranking."""
    if db.query(Player).count() > 10:
        return # Já tem players suficientes

    print("Populando banco de dados com 200 bots...")
    ...
    for i in range(200):
        ...
        name = name[:15] # Garante max 15 chars
        
        char_class = random.choice(classes)
        lvl = random.randint(1, 50) # Nível aleatório
        ...
        bot = Player(
            ...
            health=100, max_health=100, strength=10, intelligence=10, defense=10 # Stats básicos
        )
        bots.append(bot)
    
    db.add_all(bots)
    db.commit()
    print("200 Bots criados!")
