...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'menu_battle_mode':
        ...
        if player.stamina < STAMINA_COST:
            await query.answer()
            await query.message.reply_text(f"🚫 **SEM STAMINA!**\nVocê está exausto.", parse_mode='Markdown'); return
    ...
    # --- GUILD SYSTEM ---
        ...
        if data == 'menu_guild': await guild_menu_main(update, context)
            ...
            await query.answer()
            if player.gems < GUILD_CREATE_COST: await query.message.reply_text(f"🚫 **Saldo Insuficiente!**\nRequer {GUILD_CREATE_COST} Gemas.", parse_mode='Markdown'); return
            ...
            await query.answer()
            if player.guild_role == 'lider': await query.message.reply_text("🚫 Líder não pode sair.")
            else:
                g = db.query(Guild).filter(Guild.id == player.guild_id).first()
    ...
    # --- MENUS DE CONSTRUÇÃO ---
        ...
        kb = [[InlineKeyboardButton("Fazenda 🌾", callback_data='constr_fazenda'), InlineKeyboardButton("Quartel ⚔️", callback_data='constr_quartel')],
              [InlineKeyboardButton("Academia 🔮", callback_data='constr_academia'), InlineKeyboardButton("Celeiro 🏚️", callback_data='constr_celeiro')],
        ...
        if conf:
            ...
            if data.startswith('upgrade_'):
                if player.gold >= cost: player.gold -= cost; setattr(player, conf['a'], lvl+1); db.commit(); await query.answer("Up!", show_alert=True); lvl+=1; cost = int(conf['c'] * (1.5 ** lvl))
    ...
    # --- NOVO MENU DE UPGRADE (SELEÇÃO) ---
        ...
        kb = [
            ...
            [InlineKeyboardButton("🔄 Reencarnar", callback_data='respec_start')],
            [InlineKeyboardButton("🔙 Voltar", callback_data='menu_refresh')]
        ]
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')

    # --- MENU ESPECÍFICO DE TREINO (1x e 10x) ---
    elif data.startswith('train_'):
        await query.answer()
        stat_map = {'str': 'strength', 'int': 'intelligence', 'def': 'defense', 'spd': 'speed', 'crit': 'crit_chance'}
        stat_key = data.split('_')[1]
        ...
        emoji = {'str':'💪', 'int':'🧠', 'def':'🛡️', 'spd':'⚡', 'crit':'💥'}.get(stat_key, '')
        ...
        kb = [
            [InlineKeyboardButton(f"+1 ({format_number(cost_1)}g)", callback_data=f'up_{stat_key}_1'),
    ...
    # --- AÇÃO DE UPGRADE (GENÉRICA) ---
    elif data.startswith('up_'):
        # up_str_1 ou up_spd_10
        parts = data.split('_')
        stat_key = parts[1]
        qty = int(parts[2])
        ...
        temp_val = current_val
        for _ in range(qty):
            total_cost += int(50 + (temp_val * 20))
            temp_val += 1
            
        if player.gold >= total_cost:
            player.gold -= total_cost
            setattr(player, attr_name, current_val + qty)
            db.commit()
            await query.answer(f"Sucesso! +{qty} {stat_key.upper()}", show_alert=True)
            # Retorna para o menu de treino específico para continuar clicando
            # Simula o clique no botão 'train_stat'
            query.data = f"train_{stat_key}"
            await handle_menu(update, context)
        else:
            await query.answer(f"🚫 Falta Ouro! Precisa de {format_number(total_cost)}g", show_alert=True)
